...
string generateUserID() {
    ...
    // Load existing user IDs from the file
    ifstream infile("datausers.txt");
    if (infile.is_open()) {
        string username, passwordHash, userID, userProjectID;
        bool isAdmin;
        while (infile >> username >> passwordHash >> userID >> userProjectID >> isAdmin) {
            existingIDs.insert(userID);
        }
        infile.close();
    }
...
void loadUsers() {
    ...
    if (infile.is_open()) {
        User user;
        while (infile >> user.username >> user.passwordHash >> user.userID >> user.userProjectID >> user.isAdmin) {
...
void saveUsers() {
    ...
    if (outfile.is_open()) {
        for (const auto& user : users) {
            outfile << user.first << " " << user.second.passwordHash << " " << user.second.userID << " "
                    << user.second.userProjectID << " " << user.second.isAdmin << "\n";
        }
        outfile.close();
    }
}
...
void registerUser() {
    ...
    cout << "Enter Project ID to associate with: ";
    cin >> user.userProjectID;
    user.isAdmin = false;
    users[user.username] = user;
    saveUsers();
    cout << "User registered successfully. Your User ID is " << user.userID << "\n";
}
...
void menuNonAdmin(const User& user) {
    ...
    auto it = find_if(projects.begin(), projects.end(), [&user](const Project& project) {
        return project.id == user.userProjectID;
    });
    if (it == projects.end()) {
