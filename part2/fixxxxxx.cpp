...
void signalHandler(int signum) {
    saveUsers();
    saveProjects();
    cout << "\nCaught signal " << signum << ", saving data and exiting...\n";
    exit(signum);
}
...
void showProjectDetails(const Project& project) {
    ...
    if (infileTasks.is_open()) {
        ...
        string projectID, description;
        bool isComplete;
        while (infileTasks >> projectID >> ws && getline(infileTasks, description, '|') && infileTasks >> isComplete) {
            if (projectID == project.id) {
                taskQueue.push({description, isComplete});
                if (taskQueue.size() > 3) {
                    taskQueue.pop();
                }
            }
...
void addTask(Project& project) {
    ...
    if (outfile.is_open()) {
        outfile << project.id << " " << task.description << "|" << task.isComplete << "\n";
        outfile.close();
    }
...
void markTaskComplete(Project& project) {
    ...
    if (infileTasks.is_open()) {
        ...
        while (infileTasks >> projectID >> ws && getline(infileTasks, description, '|') && infileTasks >> isComplete) {
            if (projectID == project.id) {
                tasks.push_back({description, isComplete});
            }
        }
        infileTasks.close();
    }
...
void analyzeOthers(const Project& project) {
    ...
    if (infile.is_open()) {
        string projectID, description;
        bool isComplete;
        while (infile >> projectID >> ws && getline(infile, description, '|') && infile >> isComplete) {
            if (projectID == project.id) {
                cout << "Task: " << description << " - " << (isComplete ? "Complete" : "Incomplete") << "\n";
            }
        }
        infile.close();
    }
